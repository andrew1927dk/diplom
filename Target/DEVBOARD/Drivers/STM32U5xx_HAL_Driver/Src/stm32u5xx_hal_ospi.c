...
#define IS_OSPI_FUNCTIONAL_MODE(MODE)                                                                     \
    (((MODE) == OSPI_FUNCTIONAL_MODE_INDIRECT_WRITE) || ((MODE) == OSPI_FUNCTIONAL_MODE_INDIRECT_READ) || \
     ((MODE) == OSPI_FUNCTIONAL_MODE_AUTO_POLLING) || ((MODE) == OSPI_FUNCTIONAL_MODE_MEMORY_MAPPED))
...
HAL_StatusTypeDef HAL_OSPI_Init(OSPI_HandleTypeDef *hospi) {
    ...
    if (hospi == NULL) {
        ...
        if (hospi->State == HAL_OSPI_STATE_RESET) {
            ...
            MODIFY_REG(hospi->Instance->CR, OCTOSPI_CR_FTHRES,
...
void HAL_OSPI_IRQHandler(OSPI_HandleTypeDef *hospi) {
    ...
    /* OctoSPI transfer error interrupt occurred -------------------------------*/
        ...
        __HAL_OSPI_DISABLE_IT(hospi,
                              (HAL_OSPI_IT_TO | HAL_OSPI_IT_SM | HAL_OSPI_IT_FT | HAL_OSPI_IT_TC | HAL_OSPI_IT_TE));
...
HAL_StatusTypeDef HAL_OSPI_HyperbusCmd(OSPI_HandleTypeDef *hospi, OSPI_HyperbusCmdTypeDef *cmd, uint32_t Timeout) {
    ...
    if ((hospi->State == HAL_OSPI_STATE_READY) && (hospi->Init.MemoryType == HAL_OSPI_MEMTYPE_HYPERBUS)) {
        ...
        if (status == HAL_OK) {
            ...
            WRITE_REG(hospi->Instance->CCR, (cmd->DQSMode | OCTOSPI_CCR_DDTR | OCTOSPI_CCR_DMODE_2 | cmd->AddressSize |
                                             OCTOSPI_CCR_ADDTR | OCTOSPI_CCR_ADMODE_2));
    ...
    if (pData == NULL) {
        ...
        if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
            ...
            do {
                ...
                status = OSPI_WaitFlagStateUntilTimeout(hospi, (HAL_OSPI_FLAG_FT | HAL_OSPI_FLAG_TC), SET, tickstart,
                                                        Timeout);
        ...
        if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
            ...
            if (status == HAL_OK) {
                ...
                if ((hospi->hdma->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
                    ...
                    status = HAL_DMA_Start_IT(hospi->hdma, (uint32_t) pData, (uint32_t) &hospi->Instance->DR,
                                              hospi->XferSize);
                }
        ...
        if (hospi->State == HAL_OSPI_STATE_CMD_CFG) {
            ...
            if (status == HAL_OK) {
                ...
                if ((hospi->hdma->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
                    ...
                    status = HAL_DMA_Start_IT(hospi->hdma, (uint32_t) &hospi->Instance->DR, (uint32_t) pData,
                                              hospi->XferSize);
                }
                if (status == HAL_OK) {
                    /* Enable the transfer error interrupt */
...
HAL_StatusTypeDef HAL_OSPIM_Config(OSPI_HandleTypeDef *hospi, OSPIM_CfgTypeDef *cfg, uint32_t Timeout) {
    ...
    if (status == HAL_OK) {
        ...
        if ((OCTOSPIM->CR & OCTOSPIM_CR_MUXEN) != 0U) {
            ...
            if (IOM_cfg[instance].ClkPort != 0U) {
                ...
                if (IOM_cfg[instance].IOLowPort != HAL_OSPIM_IOPORT_NONE) {
                    CLEAR_BIT(OCTOSPIM->PCR[((IOM_cfg[instance].IOLowPort - 1U) & OSPI_IOM_PORT_MASK)],
                              OCTOSPIM_PCR_IOLEN);
                }
                if (IOM_cfg[instance].IOHighPort != HAL_OSPIM_IOPORT_NONE) {
                    CLEAR_BIT(OCTOSPIM->PCR[((IOM_cfg[instance].IOHighPort - 1U) & OSPI_IOM_PORT_MASK)],
                              OCTOSPIM_PCR_IOHEN);
                }
            }
        }
        ...
        if ((cfg->ClkPort == IOM_cfg[other_instance].ClkPort) || (cfg->DQSPort == IOM_cfg[other_instance].DQSPort) ||
            (cfg->NCSPort == IOM_cfg[other_instance].NCSPort) ||
        ...
        if (((cfg->Req2AckTime) >= 1U) && ((cfg->Req2AckTime) <= 256U)) {
            if ((cfg->Req2AckTime - 1U) > ((OCTOSPIM->CR & OCTOSPIM_CR_REQ2ACK_TIME) >> OCTOSPIM_CR_REQ2ACK_TIME_Pos)) {
                MODIFY_REG(OCTOSPIM->CR, OCTOSPIM_CR_REQ2ACK_TIME,
                           ((cfg->Req2AckTime - 1U) << OCTOSPIM_CR_REQ2ACK_TIME_Pos));
            } else {
                /* Nothing to do */
            }
        }
    ...
    if (cmd->InstructionMode != HAL_OSPI_INSTRUCTION_NONE) {
        if (cmd->AddressMode != HAL_OSPI_ADDRESS_NONE) {
            if (cmd->DataMode != HAL_OSPI_DATA_NONE) {
                ...
                MODIFY_REG((*ccr_reg),
                           ...
                           (cmd->InstructionMode | cmd->InstructionDtrMode | cmd->InstructionSize | cmd->AddressMode |
                            cmd->AddressDtrMode | cmd->AddressSize | cmd->DataMode | cmd->DataDtrMode));
            } else {
                ...
                MODIFY_REG((*ccr_reg),
                           ...
                           (cmd->InstructionMode | cmd->InstructionDtrMode | cmd->InstructionSize | cmd->AddressMode |
                            cmd->AddressDtrMode | cmd->AddressSize));
            ...
            *ir_reg = cmd->Instruction;
            ...
            if (cmd->DataMode != HAL_OSPI_DATA_NONE) {
                ...
                MODIFY_REG(
                    ...
                    (cmd->InstructionMode | cmd->InstructionDtrMode | cmd->InstructionSize | cmd->DataMode |
                     cmd->DataDtrMode));
            } else {
        ...
        if (cmd->AddressMode != HAL_OSPI_ADDRESS_NONE) {
            if (cmd->DataMode != HAL_OSPI_DATA_NONE) {
                ...
                MODIFY_REG(
                    (*ccr_reg),
                    (OCTOSPI_CCR_ADMODE | OCTOSPI_CCR_ADDTR | OCTOSPI_CCR_ADSIZE | OCTOSPI_CCR_DMODE |
                     OCTOSPI_CCR_DDTR),
...
static HAL_StatusTypeDef OSPIM_GetConfig(uint8_t instance_nb, OSPIM_CfgTypeDef *cfg) {
    ...
    if ((instance_nb == 0U) || (instance_nb > OSPI_NB_INSTANCE) || (cfg == NULL)) {
        ...
        if (instance_nb == 2U) {
            if ((OCTOSPIM->CR & OCTOSPIM_CR_MUXEN) == 0U) {
                value = (OCTOSPIM_PCR_CLKSRC | OCTOSPIM_PCR_DQSSRC | OCTOSPIM_PCR_NCSSRC | OCTOSPIM_PCR_IOLSRC_1 |
                         OCTOSPIM_PCR_IOHSRC_1);
            } else {
