...
UINT _tx_mutex_get(TX_MUTEX *mutex_ptr, ULONG wait_option) {
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_MUTEX_GET, mutex_ptr, wait_option,
                            TX_POINTER_TO_ULONG_CONVERT(mutex_ptr->tx_mutex_owner), mutex_ptr->tx_mutex_ownership_count,
        ...
        if (wait_option != TX_NO_WAIT) {
            ...
            if (_tx_thread_preempt_disable != ((UINT) 0)) {
                ...
                if (mutex_ptr->tx_mutex_inherit == TX_TRUE) {
                    ...
                    /* Priority inheritance is requested, check to see if the thread that owns the mutex is lower
                     * priority.  */
