...
UINT _tx_timer_deactivate(TX_TIMER *timer_ptr) {
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_TIMER_DEACTIVATE, timer_ptr, TX_POINTER_TO_ULONG_CONVERT(&ticks_left), 0, 0,
        ...
        if (active_timer_list == TX_TRUE) {
            ...
            if (TX_TIMER_INDIRECT_TO_VOID_POINTER_CONVERT(list_head) >=
                ...
                ticks_left = ticks_left +
                             (ULONG) ((TX_TIMER_POINTER_DIF(_tx_timer_list_end, _tx_timer_current_ptr)) + ((ULONG) 1));
            }
