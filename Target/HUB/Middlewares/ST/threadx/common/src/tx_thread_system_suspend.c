...
#ifndef TX_NOT_INTERRUPTABLE
{
    ...
    if (thread_ptr->tx_thread_suspending == TX_TRUE) {
        ...
        /* Log the thread status change.  */
        TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_SUSPEND, thread_ptr, thread_ptr->tx_thread_state,
...
VOID _tx_thread_system_ni_suspend(TX_THREAD *thread_ptr, ULONG wait_option) {
    ...
    /* Log the thread status change.  */
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_SUSPEND, thread_ptr, thread_ptr->tx_thread_state,
                            TX_POINTER_TO_ULONG_CONVERT(&priority), TX_POINTER_TO_ULONG_CONVERT(_tx_thread_execute_ptr),
