                    ...
                    if (priority < _tx_thread_highest_priority) {
                        ...
                        if (execute_ptr == TX_NULL) {
                            ...
                            if (priority < execute_ptr->tx_thread_preempt_threshold) {
                                ...
                                /* Now determine if preemption should take place. This is only possible if the current
                                   thread pointer is not the same as the execute thread pointer AND the system state and
...
VOID _tx_thread_system_ni_resume(TX_THREAD *thread_ptr) {
    ...
    /* Log the thread status change.  */
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_RESUME, thread_ptr, ((ULONG) thread_ptr->tx_thread_state),
                            TX_POINTER_TO_ULONG_CONVERT(&execute_ptr),
                            TX_POINTER_TO_ULONG_CONVERT(_tx_thread_execute_ptr), TX_TRACE_INTERNAL_EVENTS)
