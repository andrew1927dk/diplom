...
UINT _tx_queue_front_send(TX_QUEUE *queue_ptr, VOID *source_ptr, ULONG wait_option) {
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_QUEUE_FRONT_SEND, queue_ptr, TX_POINTER_TO_ULONG_CONVERT(source_ptr), wait_option,
        ...
        if (suspended_count == TX_NO_SUSPENSIONS) {
            ...
            if (queue_ptr->tx_queue_read == queue_ptr->tx_queue_start) {
                ...
                queue_ptr->tx_queue_read =
                    TX_ULONG_POINTER_SUB(queue_ptr->tx_queue_end, queue_ptr->tx_queue_message_size);
            } else {

                /* Not at the beginning of the queue, just move back one message.  */
                queue_ptr->tx_queue_read =
                    TX_ULONG_POINTER_SUB(queue_ptr->tx_queue_read, queue_ptr->tx_queue_message_size);
            }
    ...
    /* Determine if the caller has requested suspension.  */
        ...
        if (_tx_thread_preempt_disable != ((UINT) 0)) {
            ...
            /* Suspension is not allowed if the preempt disable flag is non-zero at this point - return error
             * completion.  */
