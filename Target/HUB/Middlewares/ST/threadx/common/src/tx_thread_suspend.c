...
UINT _tx_thread_suspend(TX_THREAD *thread_ptr) {
    ...
    TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_SUSPEND_API, thread_ptr, thread_ptr->tx_thread_state,
    ...
    if (thread_ptr->tx_thread_state == TX_READY) {
        ...
        if (status == TX_SUCCESS) {
            ...
            /* Log the thread status change.  */
            TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_SUSPEND, thread_ptr, ((ULONG) thread_ptr->tx_thread_state),
            ...
            if (ready_next != thread_ptr) {
                ...
                if (priority_map == ((ULONG) 0)) {
                    ...
                    /* Determine if preemption should take place. This is only possible if the current thread pointer is
                       not the same as the execute thread pointer AND the system state and preempt disable flags are
                       clear.  */
